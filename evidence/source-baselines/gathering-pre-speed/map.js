import {drawRegions} from './region-render.js';
...
export class MapView{
 ...
 update(state){const fresh=!this.state||this.state.seed!==state.seed||this.state.size!==state.size;this.state=state;if(fresh){this.selected=-1;this.fit();}this.build();}
 setLayer(layer){this.layer=layer;this.build();}
 build(){if(!this.state)return;const {size:n,map:m}=this.state;this.texture.width=n*3;this.texture.height=n*3;const image=this.tex.createImageData(n*3,n*3),d=image.data,layer=LAYERS.find(l=>l[0]===this.layer);for(let y=0;y<n;y++)for(let x=0;x<n;x++){const i=y*n+x,h=m.height[i],shade=(m.height[Math.max(0,y-1)*n+Math.max(0,x-1)]-m.height[Math.min(n-1,y+1)*n+Math.min(n-1,x+1)])*260;let r,g,b;if(this.layer==='natural'||this.layer==='flow'){if(h<.23||m.lakeDepth?.[i]>.012){const lake=m.lakeDepth?.[i]>.012;r=lake?105:111+h*60;g=lake?145:126+h*48;b=lake?138:125+h*45;}else{const moisture=m.water[i],temperature=m.temp[i],rock=m.rock[i],heat=m.heat[i];
 ...
 if(grassCover>0){const rgb=(grassSpecies?.color||'#789260').match(/\w\w/g).map(v=>parseInt(v,16));blend(...rgb,grassCover*.55);}
 if(treeCover>0){const rgb=(treeSpecies?.color||'#4b6648').match(/\w\w/g).map(v=>parseInt(v,16));blend(...rgb,treeCover*.5);}
 ...
 // Contours, shores and province reference lines follow the generated geometry.
 c.lineWidth=.6/z;for(let y=y0;y<y1;y++)for(let x=x0;x<x1;x++){const i=y*n+x;for(const [j,dx,dy]of [[i+1,1,0],[i+n,0,1]]){if((dx&&x===n-1)||(dy&&y===n-1))continue;let stroke=null;if(this.layer==='natural'&&this.regionMode==='off'&&Math.floor(m.height[i]*13)!==Math.floor(m.height[j]*13))stroke='#62583715';if((m.height[i]<.23)!==(m.height[j]<.23))stroke='#4d6559dd';if(this.borders&&m.height[i]>=.23&&m.height[j]>=.23&&m.province[i]!==m.province[j])stroke='#66492fe8';if(stroke){c.lineWidth=this.borders&&m.province[i]!==m.province[j]?1.25/z:.55/z;c.strokeStyle=stroke;c.beginPath();if(dx){c.moveTo(x+1,y);c.lineTo(x+1,y+1);}else{c.moveTo(x,y+1);c.lineTo(x+1,y+1);}c.stroke();}}
 ...
 if(this.layer==='natural'){
  // Overlapping ink mountain groups are positioned on the actual highland, not a regular icon grid.
  for(let gy=Math.max(3,Math.floor(y0/5)*5);gy<Math.min(n-4,y1+4);gy+=5)for(let gx=Math.max(3,Math.floor(x0/5)*5);gx<Math.min(n-4,x1+4);gx+=5){
   const offset=((gx*31+gy*17)%7)-3,x=gx+offset*.45,y=gy+((gx*13+gy*23)%5-2)*.6;
   const ix=Math.round(x),iy=Math.round(y),i=iy*n+ix,e=m.height[i];
   if(e<.60||m.river[i]>.35||m.lakeDepth?.[i]>.012)continue;
   const higher=Math.max(m.height[i-3],m.height[i+3],m.height[i-3*n],m.height[i+3*n]);
   if(e<higher-.035)continue;
   const r=1.8+(e-.60)*5,peak=r*(1.25+((gx*7+gy)%5)*.12),cx=x-.3;
   const wash=c.createLinearGradient(x,y-peak,x,y+1.3);wash.addColorStop(0,'#d8d5b4');wash.addColorStop(.6,'#a6ad8cb0');wash.addColorStop(1,'#a6ad8c00');
   c.fillStyle=wash;c.beginPath();c.moveTo(x-r*1.25,y+1);c.lineTo(x-r*.72,y-peak*.35);c.lineTo(x-r*.5,y-peak*.28);c.lineTo(cx,y-peak);c.lineTo(x+r*.27,y-peak*.40);c.lineTo(x+r*.52,y-peak*.61);c.lineTo(x+r*.84,y-peak*.18);c.lineTo(x+r*1.4,y+1.2);c.closePath();c.fill();
   c.strokeStyle='#43513fba';c.lineWidth=Math.max(.12,.7/z);c.beginPath();c.moveTo(x-r*1.2,y+.7);c.lineTo(x-r*.72,y-peak*.35);c.lineTo(x-r*.5,y-peak*.28);c.lineTo(cx,y-peak);c.lineTo(x+r*.27,y-peak*.40);c.lineTo(x+r*.52,y-peak*.61);c.lineTo(x+r*.84,y-peak*.18);c.lineTo(x+r*1.3,y+.9);c.stroke();
   c.fillStyle='#52664e50';c.beginPath();c.moveTo(cx,y-peak);c.lineTo(x-.05,y-peak*.49);c.lineTo(x+r*.2,y-.2);c.lineTo(x+r,y+.8);c.lineTo(x+r*.27,y-peak*.40);c.closePath();c.fill();
   c.strokeStyle='#52604666';c.lineWidth=Math.max(.1,.45/z);for(let b=0;b<3;b++){c.beginPath();c.moveTo(cx+r*.12*b,y-peak*(.8-b*.15));c.quadraticCurveTo(x+r*(.1+b*.14),y-peak*.25,x+r*(.35+b*.17),y+.3);c.stroke();}
   if(e>.78){c.strokeStyle='#f0edcd';c.lineWidth=Math.max(.16,.9/z);c.beginPath();c.moveTo(cx-r*.23,y-peak*.7);c.lineTo(cx,y-peak);c.lineTo(cx+r*.20,y-peak*.72);c.stroke();}
  }
