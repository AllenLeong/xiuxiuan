...
let world=null,running=false,timer=null,selected=0,speed=4,last=0,busy=false,replyTo=null,targetYear=null,lastSentRegions=null;
function send(force=false){const now=performance.now();if(!force&&now-last<200)return;last=now;world.refreshTotals();const fields=['groundEast','groundSouth','airEast','airSouth','height','river','lakeDepth','drainage','rock','heat','sun','temp','water','province','ground','air','mineral','type','count','mass','qi','stage'];const map={};for(const k of fields)map[k]=world[k];postMessage({type:'state',replyTo,size:world.size,seed:world.seed,year:world.year,steps:world.steps,config:world.config,edgeFlowStep:world.edgeFlowStep,map,...(lastSentRegions!==world.regions?{regions:world.regions}:{}),qiChannels:world.qiChannels,terrainFeatures:world.terrainFeatures,ancients:world.ancients,summary:world.summary,audit:world.lastAudit,events:world.events.slice(-60),dropped:world.droppedEvents,targetYear,selected:world.inspect(Math.min(selected,world.n-1)),running});lastSentRegions=world.regions;replyTo=null;}
