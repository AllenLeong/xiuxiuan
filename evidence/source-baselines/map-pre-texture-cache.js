import {boundaryPaths} from '../../src/map-boundaries.js';
import {drawRegions} from '../../src/region-render.js';
import {drawQiFlow} from '../../src/flow-render.js';
...
export class MapView{
 ...
 renderFrame(time){this.beforeFrame?.(time);if(this.rendering&&this.textureDirty)this.build();const drew=this.rendering&&this.dirty;if(drew)this.draw();this.onFrame?.(time,drew);this.frames.push(time);if(this.frames.length>120)this.frames.shift();}
 ...
 for(const edge of boundaryPaths(m,n,{contours:this.layer==='natural'&&this.regionMode==='off',borders:this.borders})){c.lineWidth=edge.width/z;c.strokeStyle=edge.color;c.stroke(edge.path);}
