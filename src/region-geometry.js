...
export function regionalClasses(values,land,n,passes=2,majority=5){
 ...
 if(values instanceof Uint8Array){
  for(let p=0;p<passes;p++){const next=result.slice();for(let i=0;i<result.length;i++){if(!land[i])continue;const x=i%n,y=(i/n)|0;let ones=0,zeros=0;for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){const xx=x+dx,yy=y+dy;if(xx<0||xx>=n||yy<0||yy>=n)continue;const j=yy*n+xx;if(land[j]){if(result[j])ones++;else zeros++;}}if(ones>zeros&&ones>=majority)next[i]=1;else if(zeros>ones&&zeros>=majority)next[i]=0;}result=next;}
  return result;
 }
...
export function regionRings(cells,n){
 ...
 for(const start of edges){if(seen.has(start))continue;const ring=[];let edge=start;
  while(edge&&!seen.has(edge)){seen.add(edge);ring.push(edge.a);const candidates=outgoing.get(edge.b[1]*(n+1)+edge.b[0])||[];
   let next=null;for(const turn of [1,0,3,2]){for(const candidate of candidates)if(!seen.has(candidate)&&(candidate.dir-edge.dir+4)%4===turn){next=candidate;break;}if(next)break;}edge=next;
  }
