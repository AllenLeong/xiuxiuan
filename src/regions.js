...
const terrainCache=new WeakMap(),terrainComponents=new WeakMap();
...
function flags(w,kind,old,terrain){const out=new Uint8Array(w.n);for(let i=0;i<w.n;i++){
 ...
 let cover=0;if(kind==='forest'){const k=i*2+1,s=w.species[w.type[k]];cover=w.count[k]&&s?Math.min(1,w.mass[k]*s.size/10):0;}
...
function components(mask,n,minArea,requireWidth=true,starts=null){const visited=new Uint8Array(n*n),out=[];const count=starts?starts.length:mask.length;for(let index=0;index<count;index++){const start=starts?starts[index]:index;
 if(!mask[start]||visited[start])continue;const cells=[start];visited[start]=1;let width=false;
 for(let p=0;p<cells.length;p++){const i=cells[p],x=i%n,y=Math.floor(i/n);if(requireWidth&&!width&&x>0&&y>0&&x<n-1&&y<n-1){let full=true;for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++)if(!mask[i+dy*n+dx])full=false;width ||= full;}
  if(x>0&&mask[i-1]&&!visited[i-1]){visited[i-1]=1;cells.push(i-1);}if(x<n-1&&mask[i+1]&&!visited[i+1]){visited[i+1]=1;cells.push(i+1);}if(y>0&&mask[i-n]&&!visited[i-n]){visited[i-n]=1;cells.push(i-n);}if(y<n-1&&mask[i+n]&&!visited[i+n]){visited[i+n]=1;cells.push(i+n);}}
...
export function updateRegions(w,initial=false){
 ...
 let fixed=terrainComponents.get(terrain);if(!fixed){fixed=new Map();terrainComponents.set(terrain,fixed);}
 for(const {kind,influence} of definitions){
  if(influence&&influence.cells.length<9)continue;
  ...
  let patches=fixed.get(kind);if(!patches){patches=components(influenceMask||flags(w,kind,old,terrain),w.size,influence?9:kind==='forest'?24:KINDS[kind].dimension==='qi'?12:48,!['transport','ancientDraw'].includes(kind));if(KINDS[kind].dimension==='landform')fixed.set(kind,patches);}
...
export function significantRegions(regions){
 ...
 const groups=new Map();for(const [i,at]of coverage){
  // Membership order follows the same active-region order for every cell.
  // Classify each membership group once instead of allocating sets per cell.
  const membership=at.map(r=>r.id).join('-');
  if(!groups.has(membership))groups.set(membership,{at,cells:[]});groups.get(membership).cells.push(i);
 }
 const out=[];const n=regions.size;if(!n)return out;
 for(const {at,cells}of groups.values()){
  ...
  const key=at.map(r=>r.id).sort((a,b)=>a-b).join('-');
  const mask=new Uint8Array(n*n);for(const i of cells)mask[i]=1;
  for(const piece of components(mask,n,24,true,cells.slice().sort((a,b)=>a-b))){
