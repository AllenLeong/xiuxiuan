...
// Signed distance to an irregular continental outline. Western and northern
// land continues outside the observed window; coastlines are not world edges.
function continentalDistance(x,y,points){
...
export function generateTerrain(w,noise){
 ...
 const outline=[[-.22,-.18],[.79,-.18],[.84,.06],[.94,.16],[.90,.29],[.83,.38],[.94,.49],[.91,.61],[.87,.71],[.94,.82],[.85,.96],[.72,1.10],[-.22,1.10]];
 // Three long primary ranges leave broad interior spaces; short branches
 // belong to those ranges instead of dividing every basin into small pockets.
 const chains=[[[.08,-.08],[.17,.13],[.20,.34],[.16,.52],[.22,.75],[.32,1.06]],[[.23,.08],[.40,.15],[.57,.20],[.73,.15],[.89,.06]],[[.20,.61],[.37,.58],[.56,.62],[.72,.71],[.87,.83]],[[.18,.32],[.30,.27],[.36,.24]],[[.57,.62],[.64,.51],[.70,.41]]].map((line,l)=>line.map(([x,y],j)=>[jitter(x,l*9+j,1),jitter(y,l*9+j,3)]));
 const islands=[[.89,.80,.025,.018]].map(([x,y,rx,ry],j)=>[jitter(x,80+j,1),jitter(y,80+j,3),rx,ry]);
 for(let y=0;y<n;y++)for(let x=0;x<n;x++){
  ...
  let land=continentalDistance(wx,wy,outline)*3.2;
  // Narrow headlands, open bays and an occasional coastal island.
  for(const [cx,cy,rx,ry]of islands)land=Math.max(land,ellipse(cx,cy,rx,ry)*.16);
  land+=(noise(nx*16,ny*16,seed+3)-.5)*.065+(noise(nx*35,ny*35,seed+4)-.5)*.02;
  let elevation=SEA_LEVEL+Math.min(land,.54)*.49;
  if(land>0){
   let ridge=0;
   for(let j=0;j<chains.length;j++){const d=distanceToChain(wx,wy,chains[j]),width=j<3?.032:.020;ridge=Math.max(ridge,Math.exp(-((d/width)**2))*(j<3?.47:.24));}
   ...
   for(const [cx,cy,rx,ry,depth]of [[.47,.38,.24,.16,.16],[.52,.82,.25,.14,.15]])elevation-=depth*Math.exp(-(((wx-cx)/rx)**2+((wy-cy)/ry)**2));
  }
  // An enclosed upland depression: drainage fills its floor to the lowest
  // rim outlet, leaving the resistant rock summit as a possible lake island.
  const basinRadius=Math.hypot((wx-.53)/.075,(wy-.495)/.06);
  elevation+=.10*Math.exp(-(((basinRadius-1)/.22)**2))*clamp(land*10);
  elevation-=.16*Math.exp(-((basinRadius/.72)**2))*clamp(land*10);
  ...
  // Eroded cross-range valleys open interior drainage toward the coast.
  // Elevation follows distance to the estuary, not a pre-painted river.
  for(const route of [[[.29,.35],[.44,.40],[.59,.38],[.74,.39],[.85,.40]],[[.34,.74],[.47,.81],[.64,.85],[.80,.90],[.88,.95]]]){
   const d=distanceToChain(wx,wy,route),mouth=route.at(-1),floor=SEA_LEVEL+Math.hypot(wx-mouth[0],wy-mouth[1])*.20;
   const openness=Math.exp(-((d/.020)**2));
   if(land>0)elevation=elevation*(1-openness)+Math.min(elevation,floor)*openness;
  }
